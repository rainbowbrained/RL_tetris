...
|-- pics/                       # Curated report PNG/GIF assets used in docs
...
## 11. 1500-Iteration Report Analysis

Source reports used:

- `reports/ppo_report_value_mlp_1500/report.md`
- `reports/reinforce_report_value_mlp_1500/report.md`
- Baselines for trend context:
  - `reports/ppo_report_value_mlp/report.md` (300)
  - `reports/ppo_report_value_mlp_500/report.md` (500)

### 11.1 PPO (CNN policy + MLP value): 300 -> 500 -> 1500

| Metric (last) | 300 iters | 500 iters | 1500 iters |
|---|---:|---:|---:|
| Reward | 337.63 | 540.78 | 2423.10 |
| Lines | 5.89 | 7.97 | 26.55 |
| Episode length | 29.18 | 32.91 | 62.28 |
| Fixed eval reward | 266.06 | 564.04 | 2274.50 |
| Fixed eval lines | 5.50 | 8.30 | 25.60 |
| Explained variance | 0.329 | 0.373 | 0.728 |
| Approx KL | 0.0142 | 0.0041 | 0.0050 |
| Clip fraction | 0.118 | 0.0376 | 0.0266 |
| Ratio max | 4.40 | 1.80 | 1.92 |
| Top-out rate | 0.907 | 0.903 | 0.923 |

Interpretation:

- Extending training to 1500 iterations produced a major performance jump rather than a marginal gain.
- Critic quality improved strongly (explained variance to ~0.73), supporting more reliable advantages.
- PPO updates stayed conservative late (low KL and clip fraction), consistent with the late-phase schedule (`lr_late`, `value_lr_late`, `epochs_late=2` after iteration 1000).
- Top-out rate is still high despite high score; the policy is strong but still spends most episodes near the failure boundary.

PPO 1500 core curves:

![PPO 1500 core curves](pics/ppo_cnn_mlp_1500_core_training_curves.png)

PPO 1500 diagnostics:

![PPO 1500 diagnostics](pics/ppo_cnn_mlp_1500_diagnostics.png)

PPO 1500 reward decomposition:

![PPO 1500 reward decomposition](pics/ppo_cnn_mlp_1500_reward_decomposition.png)

PPO 1500 value calibration:

![PPO 1500 value calibration](pics/ppo_cnn_mlp_1500_value_calibration.png)

### 11.2 REINFORCE at 1500 (context baseline)

| Metric (last) | REINFORCE 1500 |
|---|---:|
| Reward | -102.20 |
| Lines | 0.00 |
| Fixed eval reward | -58.41 |
| Fixed eval lines | 1.00 |

Interpretation:

- REINFORCE did not scale with longer training in this setting; final reward is worse than its initial value in this run.
- The 1500-iteration PPO improvements are therefore algorithmic (PPO + critic + update control), not just “more wall-clock”.

REINFORCE 1500 core curves:

![REINFORCE 1500 core curves](pics/reinforce_1500_core_training_curves.png)

### 11.3 Qualitative Storytelling (1500 run)

Random vs REINFORCE:

![Random vs REINFORCE (1500 run)](pics/gif_random_vs_reinforce_1500.gif)

REINFORCE vs PPO:

![REINFORCE vs PPO (1500 run)](pics/gif_reinforce_vs_ppo_1500.gif)

### 11.4 Practical Conclusion

- Yes, running longer helped, but only because late-training was made more conservative.
- The 1500 report supports continuing PPO beyond 300 when using:
  - nonlinear critic (MLP),
  - target-KL control,
  - reduced LR and epochs in late phase.
